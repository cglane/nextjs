...
export default function ProgressHeader(props: any) {
  ...
  return (
    <div className="p-4 md:flex">
      <div className="text-center text-xs font-semibold md:w-9/12 md:text-left">
        <h1>{stage.title}</h1>
      </div>
      <div className="flex md:w-6/12 ">
        <div className="w-9/12 md:w-6/12">
          <span className="text-xs font-semibold">{stage.subTitle}</span>
        </div>
        <div className="flex w-3/12 md:w-6/12">
          <StatusCheckBar currentIndex={stage.index} stages={[0, 1, 2, 3]} />
        </div>
      </div>
    </div>
  );
